...
\begin{document}
	...
	\begin{abstract}
		...
		maintaining a bounded-size external \emph{knowledge ledger} summarizing essential
		results of each session. At the end of each session, new information is appended
		as a delta, and the ledger is compressed back to a fixed text size limit. A
		\emph{Compression Difficulty Score} (CDS) quantifies how hard this compression is,
		signaling when knowledge should be externalized into permanent documents or
		implemented in durable artifacts. The protocol is vendor-agnostic: the ledger is
		plain text, interpretable by any LLM supporting text rewriting.
	\end{abstract}
	
	\section{Overview}
	
	The protocol relies on a single text file, typically named
	\texttt{context.txt}, stored with the main project artifacts. This file acts as a
	bounded, evolving representation of the external working memory accumulated
	...
	session then begins with the same distilled understanding provided by the
	ledger.
	...
	The control header appears at the top of \texttt{context.txt} and defines the
	protocol rules, constraints, and edit boundaries. It is immutable except for
	incrementing the session count.
	
	\subsection{Knowledge Region}
	
	The knowledge region is the only editable portion. It contains chronologically
	ordered session summaries, rewritten and compressed over time. Only durable
	knowledge is retained; ephemeral reasoning is discarded.
	
	\section{Renormalization Algorithm}
	
	Let $C_{\max}$ denote the fixed size budget. At the end of each session:
	
	\begin{enumerate}
		\item Increment the session counter in the header.
		\item Append a new session summary containing only new or changed durable
		knowledge.
		\item Compress the knowledge region to fit within $C_{\max}$ by merging,
		generalizing, and aggressively compressing older sessions.
		\item Compute the Compression Difficulty Score:
		\[
		CDS = \max(0, C_{\text{post,min}} - C_{\max}),
		\]
		where $C_{\text{post,min}}$ is the smallest achievable size without
		semantic loss.
	\end{enumerate}
	
	\section{Compression Difficulty Score}
	
	CDS provides an operational measure of representational pressure. A rising CDS
	signals that project knowledge should be externalized into code, documents, or
	other durable artifacts.
	
	\section{Boundedness and Project Phases}
	
	The ledger functions as bounded working memory for an active project phase.
	At major milestones, the ledger can be externalized and reset, preserving a
	compact historical trace.
	
	\section{Cross-Model and Cross-Vendor Use}
	
	Because the protocol uses plain text and explicit rules, it is portable across
	models and vendors. Any LLM capable of text rewriting can participate.
	
	\section{Related Work and Positioning}
	
	Context Renormalization intersects with conversational memory, recursive
	summarization, and external memory systems, but differs by defining a
	user-controlled, bounded, portable ledger with explicit update rules and a
	quantitative compression signal.
	
	\subsection*{Positioning}
	
	The contribution is a protocol combining:
	\begin{itemize}
		\item a fixed-size external ledger,
		\item immutable operational semantics,
		\item fair compression across sessions,
		\item and an explicit pressure signal (CDS).
	\end{itemize}
	...
	\begin{figure}[!htbp]
		...
		\caption{Renormalization-group view of Context Renormalization. The immutable
			header $H$ defines the operator $R_H$, mapping each ledger state $S_n$ to
			$S_{n+1}$.}
	\end{figure}
	...
	The protocol trades perfect recall for bounded stability. Different models may
	produce different compressions, but invariants remain preserved. Sustained CDS
	pressure indicates the need for externalization.
	
	\section{Conclusion}
	
	Context Renormalization provides a simple, portable mechanism for maintaining
	continuity across LLM sessions under strict size constraints.
	
	\appendix
	
	\section{Real Project Ledger (context.txt)}
	
	The following is the exact authoritative \texttt{context.txt} ledger (Version
	2.3) used in a live multi-session project.
